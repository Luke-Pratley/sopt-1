#ifndef SOPT_WAVELETS_SARA_MPI_H
#define SOPT_WAVELETS_SARA_MPI_H
...
class SARA_MPI : public std::vector<Wavelet_MPI> {
 public:
#ifndef SOPT_HAS_NOT_USING
  // Constructors
  using std::vector<Wavelet_MPI>::vector;
#else
  //! Default constructor
  SARA_MPI() : std::vector<Wavelet_MPI>(){};
#endif
  //! Easy constructor
  SARA_MPI(std::initializer_list<std::tuple<std::string, t_uint, mpi::Communicator>> const &init)
      : SARA_MPI(init.begin(), init.end()) {}
  ...
  template <class ITERATOR,
            class T = typename std::enable_if<
                ...
                std::is_convertible<decltype(std::get<1>(*std::declval<ITERATOR>())),
                                    t_uint>::value>::type>
  SARA_MPI(ITERATOR first, ITERATOR last) {
    for (; first != last; ++first)
      emplace_back(std::get<0>(*first), std::get<1>(*first), std::get<2>(*first));
  }

  SARA_MPI(const_iterator first, const_iterator last) : std::vector<Wavelet_MPI>(first, last) {}
  //! Destructor
  virtual ~SARA_MPI() {}
  ...
  t_uint max_levels() const {
    if (size() == 0) return 0;
    auto cmp = [](Wavelet_MPI const &a, Wavelet_MPI const &b) { return a.levels() < b.levels(); };
    return std::max_element(begin(), end(), cmp)->levels();
  }

  //! Adds a wavelet of specific type
  void emplace_back(std::string const &name, t_uint nlevels, const mpi::Communicator comm) {
    std::vector<Wavelet_MPI>::emplace_back(mpi_factory(name, nlevels, comm));
  }
};
...
template <class T0, class T1>
void SARA_MPI::direct(Eigen::ArrayBase<T1> &coeffs, Eigen::ArrayBase<T0> const &signal) const {
...
template <class T0, class T1>
void SARA_MPI::indirect(Eigen::ArrayBase<T1> const &coeffs, Eigen::ArrayBase<T0> &signal) const {
...
template <class T0>
typename T0::PlainObject SARA_MPI::indirect(Eigen::ArrayBase<T0> const &coeffs) const {
...
template <class T0>
typename T0::PlainObject SARA_MPI::direct(Eigen::ArrayBase<T0> const &signal) const {
